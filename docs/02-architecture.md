...
- Sprint 4 前三切片的 URL/Origin 风险门禁、只读动作门禁、任务状态机、最小审计事件、页面状态指纹、去重基础、指定模块入口配置、有界探索队列模型、可注入 Collector Port 的有界 Runner 编排、`SnapshotCollector` 适配器、Snapshot 1.1 脱敏 `href` 导航元数据，从快照提取去重链接导航候选的纯函数，以及 Runner 对默认候选提取和重复状态停止展开的接入；队列将无查询参数和片段的根目录 `/index.html` 规范为 `/`，避免常规首页别名重复计数；本地 HTTP fixture 已通过真实 Chromium 的两页受控端到端验收。
