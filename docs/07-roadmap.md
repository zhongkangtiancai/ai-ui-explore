...
已实现前三切片：URL/Origin 风险门禁、只读动作门禁、任务状态机、最小审计事件、
页面状态指纹、去重基础、指定模块入口配置、有界探索队列模型和可注入 Collector Port 的
有界 Runner 编排，并提供复用 Sprint 1 `SnapshotCollector` 的适配器。

尚未实现：真实浏览器多页面端到端验收、从快照自动提取导航候选、人工辅助登录运行时、
认证验证器、登录后安全检查点和本地登录集成测试站点。
