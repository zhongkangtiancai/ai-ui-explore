...
## Sprint 9：受控只读交互与流程发现（当前范围已实现）

- 已实现六类白名单只读交互、预算和默认拒绝门禁，以及步骤和状态流程图的脱敏进程内投影。
- 已实现终态 workflow API、v2 知识包字段/Schema 和 Vue 只读流程浏览。
- 仅在本地虚构站点与真实 Chromium 验收；不支持真实外站登录、业务动作、流程重放、业务权限或后端数据结论。
