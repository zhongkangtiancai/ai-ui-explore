...
## Sprint 8：统一探索知识包（当前范围已实现）

- 将当前进程内的终态任务与终态权限对比的脱敏投影按需构建为独立 v2 JSON 包。
- 提供安全来源目录、Schema 校验附件下载和 Vue 多来源选择面板；不持久化包或原始 Snapshot。
- 通过本地虚构角色站点与真实 Chromium 验证单任务探索、双身份 UI 对比和统一导出链路。
- 仍不包含 AI 推断、业务权限结论、动作/流程探索、数据库、跨进程恢复、截图、网络正文或测试代码生成。
