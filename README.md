...
它不保存原始 Snapshot、HTML、网络正文、Cookie、Token、输入值、浏览器对象或登录态，也不代表业务权限、角色授权或后端数据范围结论。任务、对比和下载结果均只存在于当前进程；重启服务后不可恢复。最多导出 5 个来源、500 页、20,000 个元素、20,000 条差异、50,000 条证据和 10 MiB JSON，超限会明确拒绝而不会静默截断。
