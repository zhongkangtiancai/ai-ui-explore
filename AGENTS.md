...
有界 Runner 编排，并提供复用 Sprint 1 `SnapshotCollector` 的适配器。Snapshot 1.1 已保存
经过 URL 脱敏的 anchor `href` 导航元数据，并可从页面快照确定性提取去重后的只读链接
导航候选。Runner 默认接入该提取器，并在重复页面状态上停止继续展开候选。当前切片
尚未进行真实浏览器多页面端到端验收，也不实现人工登录运行时。
